...
namespace Notation.Utils
{
    public static partial class PDFUtils
    {
        ...
        {
            PdfWriter writer = new PdfWriter(System.IO.Path.Combine(directory, string.Format("Bulletin de période {0} de {1} de {2} {3}.pdf", period.Number, _class.Name, student.LastName, student.FirstName)));

            PdfDocument pdfDocument = new PdfDocument(writer);
            Document document = new Document(pdfDocument);
            document.SetMargins(20, 20, 20, 20);

            GenerateReportHeader(document, student, _class, period);

            GeneratePeriodMarks(document, marks, student, period, _class);
            GeneratePeriodAverage(document, student, period);
            //GeneratePeriodComments(document, student, period);
